...
function CreditNotesList() {
  ...
  const fetchRows = useCallback(async () => {
    ...
    try {
      ...
      const { data } = await api.get<{ count: number; results: CreditNoteListRow[] }>(`/api/v1/sales/credit-notes/?${params}`);
    ...
    <div style={{ padding: '24px 28px', fontFamily: "'Heebo', sans-serif", height: '100%', overflowY: 'auto', boxSizing: 'border-box', backgroundColor: '#f4f6f8' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
        ...
        <div style={{ marginLeft: 'auto' }}>
          <button onClick={() => nav('/sales/credit-notes/add')}
      ...
      <div style={{ backgroundColor: '#fff', borderRadius: 10, border: '1px solid #efefef', boxShadow: '0 1px 4px rgba(0,0,0,0.05)', overflow: 'hidden' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={8} style={{ ...TD, textAlign: 'center', padding: '40px 0', color: '#aaa', borderRight: 'none' }}>Loading credit notes…</td></tr>
            ) : rows.length === 0 ? (
              <tr><td colSpan={8} style={{ ...TD, textAlign: 'center', padding: '40px 0', color: '#aaa', borderRight: 'none' }}>No credit notes found.</td></tr>
            ) : rows.map((r) => (
              <tr key={r.id} style={{ backgroundColor: '#fff', cursor: 'pointer' }} onClick={() => nav(`/sales/credit-notes/${r.id}`)}>
                <td style={TD}>{statusPill(r.status)}</td>
                <td style={{ ...TD, fontWeight: 600 }}>{r.credit_note_number}</td>
                <td style={TD}>{r.customer_name}</td>
                <td style={TD}>{r.date}</td>
                <td style={{ ...TD, textAlign: 'right', fontVariantNumeric: 'tabular-nums' }}>{fmt(r.total_amount)}</td>
                <td style={{ ...TD, textAlign: 'right', fontVariantNumeric: 'tabular-nums' }}>{fmt(r.refunded_amount)}</td>
...
function CreditNotesEditor() {
  ...
  const [creditNoteId, setCreditNoteId] = useState<string | null>(isCreate ? null : id || null);
  ...
  const [issuer, setIssuer] = useState<CreditNoteDetail['issuer_details'] | null>(null);
  const [qrFromApi, setQrFromApi] = useState('');
  ...
  const fetchCreditNote = useCallback(async () => {
    ...
    setError('');
    try {
      const { data } = await api.get<CreditNoteDetail>(`/api/v1/sales/credit-notes/${id}/`);
  ...
  async function confirmAndPost() {
    let targetId = creditNoteId;
    if (!targetId) {
      targetId = await saveDraft(false);
      if (!targetId) return;
      setCreditNoteId(targetId);
    }
    setPosting(true);
    setError('');
    setPostNotice('');
    ...
    <div style={{ padding: '24px 28px', fontFamily: "'Heebo', sans-serif", height: '100%', overflowY: 'auto', boxSizing: 'border-box', backgroundColor: '#f4f6f8' }}>
      <div style={{ backgroundColor: '#fff', borderRadius: 12, border: '1px solid #efefef', overflow: 'hidden' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 14px', borderBottom: '1px solid #f0f0f0' }}>
          <span style={{ fontSize: 14.5, fontWeight: 600, color: '#1a1a1a' }}>Create Credit Note</span>
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            {statusPill(status)}
            <button onClick={() => nav('/sales/credit-notes')} style={{ height: 32, paddingInline: 12, borderRadius: 7, border: '1px solid #e0e0e0', backgroundColor: '#fff', color: '#555', cursor: 'pointer', fontSize: 13.5 }}>Cancel</button>
            {canEdit && (
            ...
            {creditNoteId && canEdit && (
              <button onClick={deleteCreditNote} style={{ width: 32, height: 32, borderRadius: 7, border: '1px solid #fca5a5', backgroundColor: '#fff5f5', color: '#dc2626', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}>
                <Trash2 size={14} />
              </button>
            )}
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1.2fr) minmax(0, 0.9fr)', gap: 14, padding: 14 }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div style={{ display: 'grid', gridTemplateColumns: '110px minmax(0, 1fr)', gap: 10, alignItems: 'center' }}>
              <span style={{ fontSize: 20, fontWeight: 700, color: '#111827', gridColumn: '1 / span 2' }}>Credit Note <span style={{ fontSize: 12, fontWeight: 500, color: '#6b7280' }}># {creditNoteNumber || 'CN-INV100101'}</span></span>
...
export default function CreditNotes() {
  const { id } = useParams<{ id?: string }>();
  if (id) return <CreditNotesEditor />;
